...
{
    public class AccountPageBusiness(EasyComptaContext context, IAccountEnterBusiness accountEnterBusiness, IPaymentDoneBusiness paymentDoneBusiness) : BaseBusiness<AccountPage>(context), IAccountPageBusiness
    {
        ...
        public async Task<AccountPage> GetPageByDate(int month, int year) {
            ...
            {
                var page = await this._context.AccountPages.Where(page => page.Date.Month == month && page.Date.Year == year)
                    .FirstOrDefaultAsync() 
                    ?? new AccountPage();
