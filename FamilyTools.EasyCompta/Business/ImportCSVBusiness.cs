...
{
    private readonly List<AccountEnter> _accountEnters = [];
    ...
    private readonly List<CSVCA> _csvCAs = [];
    ...
    {
        var accountEnterGroupByMouthAndYear = this._accountEnters.GroupBy(x => new { x.Date.Year, x.Date.Month })
    ...
    private async Task ChangeInAccountEnter()
    {
        ...
        foreach (var csvca in this._csvCAs)
        {
            float totalValue = 0;
            ...
            this._accountEnters.Add(new AccountEnter(csvca.Libelle, csvca.OperationType,
                DateOnly.FromDateTime(csvca.Date),
                totalValue, lines, defaultTag));
        }
    }
        ...
        try
        {
            ...
            while (await r.ReadAsync())
                try
                {
                    ...
                    if (csvca.Date != DateTime.MinValue) this._csvCAs.Add(csvca);
                }
                catch (Exception ex)
                {
                    this._logger.LogWarning(
            ...
            this._logger.LogInformation($"Successfully parsed {this._csvCAs.Count} transactions");
        }
        catch (Exception ex)
        {
    ...
    {
        if (string.IsNullOrEmpty(this._csvContent)) return;

        this.RemoveFirstLines(11);
